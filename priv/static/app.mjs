...
function makeError(variant, module, line, fn, message, extra) {
  let error5 = new globalThis.Error(message);
  error5.gleam_error = variant;
  error5.module = module;
  error5.line = line;
  error5.fn = fn;
  for (let k in extra)
    error5[k] = extra[k];
...
function push_path(error5, name) {
  ...
  return error5.withFields({ path: prepend(name$2, error5.path) });
}
...
var Element = class extends CustomType {
  constructor(key, namespace2, tag3, attrs, children, self_closing, void$) {
    super();
    this.key = key;
...
function placeholder(text4) {
  return attribute("placeholder", text4);
}
...
function element(tag3, attrs, children) {
  if (tag3 === "area") {
    return new Element("", "", tag3, attrs, toList([]), false, true);
  } else if (tag3 === "base") {
    return new Element("", "", tag3, attrs, toList([]), false, true);
  } else if (tag3 === "br") {
    return new Element("", "", tag3, attrs, toList([]), false, true);
    ...
    return new Element("", "", tag3, attrs, children, false, false);
  }
}
function namespaced(namespace2, tag3, attrs, children) {
  return new Element("", namespace2, tag3, attrs, children, false, false);
}
...
function morph(prev, next, dispatch, isComponent = false) {
  let out;
  let stack4 = [{ prev, next, parent: prev.parentNode }];
  while (stack4.length) {
    let { prev: prev2, next: next2, parent } = stack4.pop();
...
function createElementNode({ prev, next, dispatch, stack: stack4 }) {
  ...
  for (const child of next.children) {
    if (child.key !== void 0 && seenKeys !== null) {
      ...
      if (keyedChildren.size === 0) {
        stack4.unshift({ prev: prevChild, next: child, parent: el2 });
        prevChild = prevChild?.nextSibling;
        continue;
      }
      if (seenKeys.has(child.key)) {
        console.warn(`Duplicate key found in Lustre vnode: ${child.key}`);
        stack4.unshift({ prev: null, next: child, parent: el2 });
        continue;
      }
      ...
      if (!keyedChild && prevChild !== null) {
        ...
        stack4.unshift({ prev: placeholder2, next: child, parent: el2 });
        continue;
      }
      ...
      stack4.unshift({ prev: keyedChild, next: child, parent: el2 });
    } else {
      stack4.unshift({ prev: prevChild, next: child, parent: el2 });
      prevChild = prevChild?.nextSibling;
    }
...
function lustreServerEventHandler(event2) {
  ...
  const tag3 = el2.getAttribute(`data-lustre-on-${event2.type}`);
...
var LustreClientApplication2 = class _LustreClientApplication {
  ...
  #update = null;
  #view = null;
  static start(flags, selector, init3, update4, view2) {
    if (!is_browser())
      return new Error(new NotABrowser());
    const root2 = selector instanceof HTMLElement ? selector : document.querySelector(selector);
    if (!root2)
      return new Error(new ElementNotFound(selector));
    const app = new _LustreClientApplication(init3(flags), update4, view2, root2);
...
function simple(init3, update4, view2) {
  let init$1 = (flags) => {
...
// build/dev/javascript/lustre_ui/lustre/ui/breadcrumbs.mjs
function of2(element2, attributes, separator, children) {
  return element2(
    prepend(class$("lustre-ui-breadcrumbs"), attributes),
    intersperse(children, separator)
  );
}
function breadcrumbs(attributes, separator, children) {
  return of2(ol, attributes, separator, children);
}
function active() {
  return class$("active");
}
...
function of4(element2, attributes, label2, input5, message) {
  return of3(
    element2,
    prepend(
...
function field2(attributes, label2, input5, message) {
  return of4(label, attributes, label2, input5, message);
}
function label_centre() {
  return class$("label-centre");
}
function message_start() {
  return class$("message-start");
}
function error3() {
  return attribute("data-variant", "error");
}
...
function of5(element2, attributes, side, main2) {
  return element2(
...
function aside(attributes, side, main2) {
  return of5(div, attributes, side, main2);
}
...
function of6(element2, attributes, children) {
  return element2(
...
function centre(attributes, children) {
  return of6(div, attributes, children);
}

// build/dev/javascript/lustre_ui/lustre/ui/layout/cluster.mjs
function of7(element2, attributes, children) {
  return element2(
...
function cluster(attributes, children) {
  return of7(div, attributes, children);
}
function stretch() {
...
function of8(element2, attributes, children) {
  return element2(
...
function group(attributes, children) {
  return of8(div, attributes, children);
}

// build/dev/javascript/lustre_ui/lustre/ui/prose.mjs
function of9(element2, attributes, children) {
  return element2(
...
function prose(attributes, children) {
  return of9(div, attributes, children);
}
function full() {
  return class$("full");
}

// build/dev/javascript/lustre_ui/lustre/ui/tag.mjs
function of10(element2, attributes, children) {
  return element2(
    prepend(class$("lustre-ui-tag"), attributes),
    children
  );
}
function tag(attributes, children) {
  return of10(span, attributes, children);
}
...
function from_rgb255(red3, green3, blue3) {
  return then$(
    (() => {
      let _pipe = red3;
...
// build/dev/javascript/lustre_ui/lustre/ui/util/colour.mjs
...
var Theme = class extends CustomType {
  constructor(space, text4, radius, primary, greyscale2, error5, warning, success2, info) {
    super();
    this.space = space;
    this.text = text4;
    this.radius = radius;
...
var Size = class extends CustomType {
...
function font_letter_uppercase(attrs) {
  return icon(
    attrs,
...
function font_letter_capitalcase(attrs) {
  return icon(
    attrs,
    "M3.68979 2.75C3.89667 2.74979 4.08232 2.87701 4.15679 3.07003L7.36662 11.39C7.46602 11.6477 7.33774 11.9371 7.0801 12.0365C6.82247 12.1359 6.53304 12.0076 6.43365 11.75L5.3825 9.02537H2.01133L0.966992 11.749C0.868128 12.0068 0.578964 12.1357 0.321126 12.0369C0.0632878 11.938 -0.0655864 11.6488 0.0332774 11.391L3.22344 3.07099C3.29751 2.87782 3.4829 2.75021 3.68979 2.75ZM3.69174 4.64284L5.05458 8.17537H2.33724L3.69174 4.64284ZM10.8989 5.20703C9.25818 5.20703 8.00915 6.68569 8.00915 8.60972C8.00915 10.6337 9.35818 12.0124 10.8989 12.0124C11.7214 12.0124 12.5744 11.6692 13.1543 11.0219V11.53C13.1543 11.7785 13.3557 11.98 13.6043 11.98C13.8528 11.98 14.0543 11.7785 14.0543 11.53V5.72C14.0543 5.47147 13.8528 5.27 13.6043 5.27C13.3557 5.27 13.1543 5.47147 13.1543 5.72V6.22317C12.6054 5.60095 11.7924 5.20703 10.8989 5.20703ZM13.1543 9.79823V7.30195C12.7639 6.58101 11.9414 6.05757 11.0868 6.05757C10.1088 6.05757 9.03503 6.96581 9.03503 8.60955C9.03503 10.1533 10.0088 11.1615 11.0868 11.1615C11.9701 11.1615 12.7719 10.4952 13.1543 9.79823Z"
  );
}
function font_letter_lowercase(attrs) {
  return icon(
    attrs,
    "M3.69899 5.20703C2.05823 5.20703 0.809204 6.68569 0.809204 8.60972C0.809204 10.6337 2.15823 12.0124 3.69899 12.0124C4.52126 12.0124 5.37402 11.6693 5.9539 11.0223V11.53C5.9539 11.7785 6.15537 11.98 6.4039 11.98C6.65243 11.98 6.8539 11.7785 6.8539 11.53V5.72001C6.8539 5.47149 6.65243 5.27001 6.4039 5.27001C6.15537 5.27001 5.9539 5.47149 5.9539 5.72001V6.22269C5.40506 5.60075 4.59218 5.20703 3.69899 5.20703ZM5.9539 9.799V7.30117C5.56339 6.58064 4.74118 6.05757 3.8868 6.05757C2.9089 6.05757 1.83508 6.96581 1.83508 8.60955C1.83508 10.1533 2.80889 11.1615 3.8868 11.1615C4.76984 11.1615 5.57141 10.4957 5.9539 9.799ZM10.799 5.20703C9.15823 5.20703 7.9092 6.68569 7.9092 8.60972C7.9092 10.6337 9.25823 12.0124 10.799 12.0124C11.6215 12.0124 12.4744 11.6692 13.0543 11.0218V11.53C13.0543 11.7785 13.2558 11.98 13.5043 11.98C13.7529 11.98 13.9543 11.7785 13.9543 11.53V5.72C13.9543 5.47147 13.7529 5.27 13.5043 5.27C13.2558 5.27 13.0543 5.47147 13.0543 5.72V6.22318C12.5055 5.60095 11.6924 5.20703 10.799 5.20703ZM13.0543 9.79822V7.30196C12.664 6.58102 11.8415 6.05757 10.9868 6.05757C10.0089 6.05757 8.93508 6.96581 8.93508 8.60955C8.93508 10.1533 9.90889 11.1615 10.9868 11.1615C11.8702 11.1615 12.672 10.4952 13.0543 9.79822Z"
  );
}
function font_letter_case_toggle(attrs) {
  return icon(
    attrs,
...
function arrow_top_right(attrs) {
  return icon(
    attrs,
...
function double_arrow_down(attrs) {
  return icon(
    attrs,
...
function double_arrow_left(attrs) {
  return icon(
    attrs,
...
function double_arrow_right(attrs) {
  return icon(
    attrs,
...
function double_arrow_up(attrs) {
  return icon(
    attrs,
...
function thick_arrow_left(attrs) {
  return icon(
    attrs,
...
function thick_arrow_right(attrs) {
  return icon(
    attrs,
...
function thick_arrow_down(attrs) {
  return icon(
    attrs,
...
function triangle_left(attrs) {
  return icon(attrs, "M9 4L9 11L4.5 7.5L9 4Z");
}
function triangle_right(attrs) {
  return icon(attrs, "M6 11L6 4L10.5 7.5L6 11Z");
}
function triangle_up(attrs) {
  return icon(attrs, "M4 9H11L7.5 4.5L4 9Z");
}
function triangle_down(attrs) {
  return icon(attrs, "M4 6H11L7.5 10.5L4 6Z");
}
...
function component_instance(attrs) {
  return icon(
    attrs,
...
function component_boolean(attrs) {
  return icon(
    attrs,
...
function shadow_inner(attrs) {
  return icon(
    attrs,
    "M0.877075 7.49988C0.877075 3.84219 3.84222 0.877045 7.49991 0.877045C11.1576 0.877045 14.1227 3.84219 14.1227 7.49988C14.1227 11.1575 11.1576 14.1227 7.49991 14.1227C3.84222 14.1227 0.877075 11.1575 0.877075 7.49988ZM7.49991 1.82704C4.36689 1.82704 1.82708 4.36686 1.82708 7.49988C1.82708 10.6329 4.36689 13.1727 7.49991 13.1727C10.6329 13.1727 13.1727 10.6329 13.1727 7.49988C13.1727 4.36686 10.6329 1.82704 7.49991 1.82704Z"
  );
}
function shadow_outer(attrs) {
  return icon(
    attrs,
...
function value2(attrs) {
  return icon(
    attrs,
    "M0.877075 7.49988C0.877075 3.84219 3.84222 0.877045 7.49991 0.877045C11.1576 0.877045 14.1227 3.84219 14.1227 7.49988C14.1227 11.1575 11.1576 14.1227 7.49991 14.1227C3.84222 14.1227 0.877075 11.1575 0.877075 7.49988ZM7.49991 1.82704C4.36689 1.82704 1.82708 4.36686 1.82708 7.49988C1.82708 10.6329 4.36689 13.1727 7.49991 13.1727C10.6329 13.1727 13.1727 10.6329 13.1727 7.49988C13.1727 4.36686 10.6329 1.82704 7.49991 1.82704Z"
  );
}
function value_none(attrs) {
  return icon(
    attrs,
...
function transparency_grid(attrs) {
  return icon(
    attrs,
...
function rotate_counter_clockwise(attrs) {
  return icon(
    attrs,
...
function column_spacing(attrs) {
  return icon(
    attrs,
    "M8.00012 1.5C8.00012 1.22386 7.77626 1 7.50012 1C7.22398 1 7.00012 1.22386 7.00012 1.5V13.5C7.00012 13.7761 7.22398 14 7.50012 14C7.77626 14 8.00012 13.7761 8.00012 13.5V1.5ZM3.31812 5.818C3.49386 5.64227 3.49386 5.35734 3.31812 5.18161C3.14239 5.00587 2.85746 5.00587 2.68173 5.18161L0.681729 7.18161C0.505993 7.35734 0.505993 7.64227 0.681729 7.818L2.68173 9.818C2.85746 9.99374 3.14239 9.99374 3.31812 9.818C3.49386 9.64227 3.49386 9.35734 3.31812 9.18161L2.08632 7.9498H5.50017C5.7487 7.9498 5.95017 7.74833 5.95017 7.4998C5.95017 7.25128 5.7487 7.0498 5.50017 7.0498H2.08632L3.31812 5.818ZM12.3181 5.18161C12.1424 5.00587 11.8575 5.00587 11.6817 5.18161C11.506 5.35734 11.506 5.64227 11.6817 5.818L12.9135 7.0498H9.50017C9.25164 7.0498 9.05017 7.25128 9.05017 7.4998C9.05017 7.74833 9.25164 7.9498 9.50017 7.9498H12.9135L11.6817 9.18161C11.506 9.35734 11.506 9.64227 11.6817 9.818C11.8575 9.99374 12.1424 9.99374 12.3181 9.818L14.3181 7.818C14.4939 7.64227 14.4939 7.35734 14.3181 7.18161L12.3181 5.18161Z"
  );
}
function row_spacing(attrs) {
  return icon(
    attrs,
    "M7.81832 0.68179C7.64258 0.506054 7.35766 0.506054 7.18192 0.68179L5.18192 2.68179C5.00619 2.85753 5.00619 3.14245 5.18192 3.31819C5.35766 3.49392 5.64258 3.49392 5.81832 3.31819L7.05012 2.08638L7.05012 5.50023C7.05012 5.74876 7.25159 5.95023 7.50012 5.95023C7.74865 5.95023 7.95012 5.74876 7.95012 5.50023L7.95012 2.08638L9.18192 3.31819C9.35766 3.49392 9.64258 3.49392 9.81832 3.31819C9.99406 3.14245 9.99406 2.85753 9.81832 2.68179L7.81832 0.68179ZM7.95012 12.9136V9.50023C7.95012 9.2517 7.74865 9.05023 7.50012 9.05023C7.25159 9.05023 7.05012 9.2517 7.05012 9.50023V12.9136L5.81832 11.6818C5.64258 11.5061 5.35766 11.5061 5.18192 11.6818C5.00619 11.8575 5.00619 12.1424 5.18192 12.3182L7.18192 14.3182C7.26632 14.4026 7.38077 14.45 7.50012 14.45C7.61947 14.45 7.73393 14.4026 7.81832 14.3182L9.81832 12.3182C9.99406 12.1424 9.99406 11.8575 9.81832 11.6818C9.64258 11.5061 9.35766 11.5061 9.18192 11.6818L7.95012 12.9136ZM1.49994 7.00017C1.2238 7.00017 0.999939 7.22403 0.999939 7.50017C0.999939 7.77631 1.2238 8.00017 1.49994 8.00017L13.4999 8.00017C13.7761 8.00017 13.9999 7.77631 13.9999 7.50017C13.9999 7.22403 13.7761 7.00017 13.4999 7.00017L1.49994 7.00017Z"
  );
}
function play(attrs) {
  return icon(
...
function question_mark_circled(attrs) {
  return icon(
    attrs,
...
function open_in_new_window(attrs) {
  return icon(
    attrs,
...
function drag_handle_vertical(attrs) {
  return icon(
    attrs,
...
function drag_handle_horizontal(attrs) {
  return icon(
    attrs,
...
function drag_handle_dots_alt(attrs) {
  return icon(
    attrs,
...
function circle_backslash(attrs) {
  return icon(
    attrs,
...
function view_horizontal(attrs) {
  return icon(
    attrs,
...
function square(attrs) {
  return icon(
    attrs,
    "M1 1H1.5H13.5H14V1.5V13.5V14H13.5H1.5H1V13.5V1.5V1ZM2 2V13H13V2H2Z"
  );
}
function copy(attrs) {
  return icon(
...
function envelope_closed(attrs) {
  return icon(
    attrs,
...
function link_break(attrs) {
  return icon(
    attrs,
    "M13.3536 2.35355C13.5488 2.15829 13.5488 1.84171 13.3536 1.64645C13.1583 1.45118 12.8417 1.45118 12.6464 1.64645L1.64645 12.6464C1.45118 12.8417 1.45118 13.1583 1.64645 13.3536C1.84171 13.5488 2.15829 13.5488 2.35355 13.3536L13.3536 2.35355ZM2.03735 8.46678C2.17398 9.12619 2.66918 9.67103 3.33886 9.89338L2.57833 10.6539C1.80843 10.2534 1.23784 9.53693 1.05815 8.66967C0.999538 8.38681 0.999604 8.06004 0.999703 7.56313L0.999711 7.50001L0.999703 7.43689C0.999604 6.93998 0.999538 6.61321 1.05815 6.33035C1.29846 5.17053 2.2379 4.28039 3.4182 4.055C3.70687 3.99988 4.04134 3.99993 4.56402 4.00001L4.62471 4.00001H5.49971C5.77585 4.00001 5.99971 4.22387 5.99971 4.50001C5.99971 4.77615 5.77585 5.00001 5.49971 5.00001H4.62471C4.02084 5.00001 3.78907 5.00225 3.60577 5.03725C2.80262 5.19062 2.19157 5.78895 2.03735 6.53324C2.00233 6.70225 1.99971 6.91752 1.99971 7.50001C1.99971 8.08251 2.00233 8.29778 2.03735 8.46678ZM12.9621 6.53324C12.8255 5.87397 12.3304 5.32922 11.661 5.10679L12.4215 4.34631C13.1912 4.74686 13.7616 5.46323 13.9413 6.33035C13.9999 6.61321 13.9998 6.93998 13.9997 7.43688L13.9997 7.50001L13.9997 7.56314C13.9998 8.06005 13.9999 8.38681 13.9413 8.66967C13.701 9.8295 12.7615 10.7196 11.5812 10.945C11.2925 11.0001 10.9581 11.0001 10.4354 11L10.3747 11H9.49971C9.22357 11 8.99971 10.7762 8.99971 10.5C8.99971 10.2239 9.22357 10 9.49971 10H10.3747C10.9786 10 11.2104 9.99777 11.3937 9.96277C12.1968 9.8094 12.8079 9.21108 12.9621 8.46678C12.9971 8.29778 12.9997 8.08251 12.9997 7.50001C12.9997 6.91752 12.9971 6.70225 12.9621 6.53324Z"
  );
}
function link_alt(attrs) {
  return icon(
    attrs,
...
function bookmark_filled(attrs) {
  return icon(
    attrs,
...
function sewing_pin_filled(attrs) {
  return icon(
    attrs,
...
function crumpled_paper(attrs) {
  return icon(
    attrs,
    "M4.60913 0.0634287C4.39082 0.0088505 4.16575 0.12393 4.08218 0.332867L3.1538 2.6538L0.832866 3.58218C0.702884 3.63417 0.604504 3.7437 0.566705 3.87849C0.528906 4.01329 0.555994 4.158 0.639992 4.26999L2.01148 6.09864L1.06343 9.89085C1.00944 10.1068 1.12145 10.3298 1.32691 10.4154L4.20115 11.613L5.62557 13.7496C5.73412 13.9124 5.93545 13.9864 6.12362 13.9327L9.62362 12.9327C9.62988 12.9309 9.63611 12.929 9.64229 12.9269L12.6423 11.9269C12.7923 11.8769 12.905 11.7519 12.9393 11.5976L13.9393 7.09761C13.9776 6.92506 13.9114 6.74605 13.77 6.63999L11.95 5.27499V2.99999C11.95 2.82955 11.8537 2.67373 11.7012 2.5975L8.70124 1.0975C8.67187 1.08282 8.64098 1.07139 8.60913 1.06343L4.60913 0.0634287ZM11.4323 6.01173L12.7748 7.01858L10.2119 9.15429C10.1476 9.20786 10.0995 9.2783 10.0731 9.35769L9.25382 11.8155L7.73849 10.8684C7.52774 10.7367 7.25011 10.8007 7.11839 11.0115C6.98667 11.2222 7.05074 11.4999 7.26149 11.6316L8.40341 12.3453L6.19221 12.9771L4.87441 11.0004C4.82513 10.9265 4.75508 10.8688 4.67307 10.8346L2.03046 9.73352L2.85134 6.44999H4.99999C5.24852 6.44999 5.44999 6.24852 5.44999 5.99999C5.44999 5.75146 5.24852 5.54999 4.99999 5.54999H2.72499L1.7123 4.19974L3.51407 3.47903L6.35769 4.4269C6.53655 4.48652 6.73361 4.42832 6.85138 4.28111L8.62413 2.06518L11.05 3.27811V5.19533L8.83287 6.08218C8.70996 6.13134 8.61494 6.23212 8.57308 6.35769L8.07308 7.85769C7.99449 8.09346 8.12191 8.34831 8.35769 8.4269C8.59346 8.50549 8.84831 8.37807 8.9269 8.14229L9.3609 6.84029L11.4323 6.01173ZM7.71052 1.76648L6.34462 3.47386L4.09505 2.724L4.77192 1.03183L7.71052 1.76648ZM10.2115 11.7885L12.116 11.1537L12.7745 8.19034L10.8864 9.76374L10.2115 11.7885Z"
  );
}
function mix(attrs) {
  return icon(
    attrs,
    "M2.14921 3.99996C2.14921 2.97778 2.97784 2.14915 4.00002 2.14915C5.02219 2.14915 5.85083 2.97778 5.85083 3.99996C5.85083 5.02213 5.02219 5.85077 4.00002 5.85077C2.97784 5.85077 2.14921 5.02213 2.14921 3.99996ZM4.00002 1.24915C2.48079 1.24915 1.24921 2.48073 1.24921 3.99996C1.24921 5.51919 2.48079 6.75077 4.00002 6.75077C5.51925 6.75077 6.75083 5.51919 6.75083 3.99996C6.75083 2.48073 5.51925 1.24915 4.00002 1.24915ZM5.82034 11.0001L2.49998 12.8369V9.16331L5.82034 11.0001ZM2.63883 8.21159C2.17228 7.9535 1.59998 8.29093 1.59998 8.82411V13.1761C1.59998 13.7093 2.17228 14.0467 2.63883 13.7886L6.57235 11.6126C7.05389 11.3462 7.05389 10.654 6.57235 10.3876L2.63883 8.21159ZM8.30001 9.00003C8.30001 8.61343 8.61341 8.30003 9.00001 8.30003H13C13.3866 8.30003 13.7 8.61343 13.7 9.00003V13C13.7 13.3866 13.3866 13.7 13 13.7H9.00001C8.61341 13.7 8.30001 13.3866 8.30001 13V9.00003ZM9.20001 9.20003V12.8H12.8V9.20003H9.20001ZM13.4432 2.19311C13.6189 2.01737 13.6189 1.73245 13.4432 1.55671C13.2675 1.38098 12.9826 1.38098 12.8068 1.55671L11 3.36353L9.19321 1.55674C9.01748 1.381 8.73255 1.381 8.55682 1.55674C8.38108 1.73247 8.38108 2.0174 8.55682 2.19313L10.3636 3.99992L8.55682 5.80671C8.38108 5.98245 8.38108 6.26737 8.55682 6.44311C8.73255 6.61885 9.01748 6.61885 9.19321 6.44311L11 4.63632L12.8068 6.44314C12.9826 6.61887 13.2675 6.61887 13.4432 6.44314C13.6189 6.2674 13.6189 5.98247 13.4432 5.80674L11.6364 3.99992L13.4432 2.19311Z"
  );
}
function mixer_horizontal(attrs) {
  return icon(
    attrs,
    "M5.5 3C4.67157 3 4 3.67157 4 4.5C4 5.32843 4.67157 6 5.5 6C6.32843 6 7 5.32843 7 4.5C7 3.67157 6.32843 3 5.5 3ZM3 5C3.01671 5 3.03323 4.99918 3.04952 4.99758C3.28022 6.1399 4.28967 7 5.5 7C6.71033 7 7.71978 6.1399 7.95048 4.99758C7.96677 4.99918 7.98329 5 8 5H13.5C13.7761 5 14 4.77614 14 4.5C14 4.22386 13.7761 4 13.5 4H8C7.98329 4 7.96677 4.00082 7.95048 4.00242C7.71978 2.86009 6.71033 2 5.5 2C4.28967 2 3.28022 2.86009 3.04952 4.00242C3.03323 4.00082 3.01671 4 3 4H1.5C1.22386 4 1 4.22386 1 4.5C1 4.77614 1.22386 5 1.5 5H3ZM11.9505 10.9976C11.7198 12.1399 10.7103 13 9.5 13C8.28967 13 7.28022 12.1399 7.04952 10.9976C7.03323 10.9992 7.01671 11 7 11H1.5C1.22386 11 1 10.7761 1 10.5C1 10.2239 1.22386 10 1.5 10H7C7.01671 10 7.03323 10.0008 7.04952 10.0024C7.28022 8.8601 8.28967 8 9.5 8C10.7103 8 11.7198 8.8601 11.9505 10.0024C11.9668 10.0008 11.9833 10 12 10H13.5C13.7761 10 14 10.2239 14 10.5C14 10.7761 13.7761 11 13.5 11H12C11.9833 11 11.9668 10.9992 11.9505 10.9976ZM8 10.5C8 9.67157 8.67157 9 9.5 9C10.3284 9 11 9.67157 11 10.5C11 11.3284 10.3284 12 9.5 12C8.67157 12 8 11.3284 8 10.5Z"
  );
}
function mixer_vertical(attrs) {
  return icon(
    attrs,
...
function card_stack_plus(attrs) {
  return icon(
    attrs,
...
function card_stack_minus(attrs) {
  return icon(
    attrs,
...
function counter_clockwise_clock(attrs) {
  return icon(
    attrs,
...
function countdown_timer(attrs) {
  return icon(
    attrs,
...
function divider_vertical(attrs) {
  return icon(
    attrs,
...
function divider_horizontal(attrs) {
  return icon(
    attrs,
...
function corner_top_right(attrs) {
  return icon(
    attrs,
...
function corner_bottom_left(attrs) {
  return icon(
    attrs,
...
function corner_bottom_right(attrs) {
  return icon(
    attrs,
...
function alignment_centre_vertically(attrs) {
  return icon(
    attrs,
...
function alignment_bottom(attrs) {
  return icon(
    attrs,
...
function stretch_vertically(attrs) {
  return icon(
    attrs,
...
function alignment_centre_horizontally(attrs) {
  return icon(
    attrs,
...
function space_between_horizontally(attrs) {
  return icon(
    attrs,
...
function space_even_horizontally(attrs) {
  return icon(
    attrs,
...
function space_between_vertically(attrs) {
  return icon(
    attrs,
...
function space_even_vertically(attrs) {
  return icon(
    attrs,
...
function value_to_css(value3) {
  if (value3 instanceof Px) {
    let val = value3[0];
    return to_string4(val) + "px";
  } else if (value3 instanceof Rem) {
    let val = value3[0];
    return to_string4(val) + "rem";
  } else {
    let name = value3[0];
    return "var(" + name + ")";
  }
}
function size_to_dict(size2, prefix) {
  return from_list(
    toList([
      ["$(" + prefix + ".base)", value_to_css(size2.base)],
      ["$(" + prefix + ".ratio)", to_string4(size2.ratio)]
    ])
  );
}
function scale_to_dict(scale, prefix) {
  ...
  return from_list(map2(keys2, vals, new$2));
}
function theme_to_dict(theme) {
  let _pipe = new$();
  let _pipe$1 = merge(_pipe, size_to_dict(theme.space, "space"));
  let _pipe$2 = merge(_pipe$1, size_to_dict(theme.text, "text"));
  let _pipe$3 = insert(
    _pipe$2,
    "$(border.radius)",
    value_to_css(theme.radius)
  );
  let _pipe$4 = merge(_pipe$3, scale_to_dict(theme.primary, "primary"));
  let _pipe$5 = merge(
    _pipe$4,
    scale_to_dict(theme.greyscale, "greyscale")
  );
  let _pipe$6 = merge(_pipe$5, scale_to_dict(theme.error, "error"));
  let _pipe$7 = merge(_pipe$6, scale_to_dict(theme.success, "success"));
  let _pipe$8 = merge(_pipe$7, scale_to_dict(theme.warning, "warning"));
  return merge(_pipe$8, scale_to_dict(theme.info, "info"));
}
...
function scoped(theme, selector) {
  let _pipe = theme_to_dict(theme);
  let _pipe$1 = fold(_pipe, theme_css, replace2);
  let _pipe$2 = replace2(_pipe$1, ":root", selector);
  return ((_capture) => {
...
var UserChangedSpaceBase = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var UserChangedSpaceRatio = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var UserChangedTextBase = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var UserChangedTextRatio = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var UserChangedRadius = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
...
function init2(_) {
  return new Theme(
    new Size(new Rem(1.5), 1.618),
    new Size(new Rem(1.125), 1.215),
    new Px(15),
    iris(),
    slate(),
    red2(),
...
function update3(theme, msg) {
  if (msg instanceof UserChangedSpaceBase) {
    let value3 = msg[0];
    return theme.withFields({
      space: theme.space.withFields({ base: new Rem(value3) })
    });
  } else if (msg instanceof UserChangedSpaceRatio) {
    let value3 = msg[0];
    return theme.withFields({ space: theme.space.withFields({ ratio: value3 }) });
  } else if (msg instanceof UserChangedTextBase) {
    let value3 = msg[0];
    return theme.withFields({
      text: theme.text.withFields({ base: new Rem(value3) })
    });
  } else if (msg instanceof UserChangedTextRatio) {
    let value3 = msg[0];
    return theme.withFields({ text: theme.text.withFields({ ratio: value3 }) });
  } else {
    let value3 = msg[0];
    return theme.withFields({ radius: new Px(value3) });
  }
}
function view(theme) {
  let styles = toList([
  ...
  return div(
    ...
    toList([
      ...
      stack2(
        toList([id("container"), style(styles)]),
        toList([
          aside2(
            toList([content_last()]),
            input(
              toList([
                ...
                on2(
                  ...
                  (event2) => {
                    ...
                    let value3 = $[0];
                    let $1 = (() => {
                      let _pipe = parse2(value3);
                      return lazy_or(
                        _pipe,
                        () => {
                          let _pipe$1 = parse(value3);
                          return map3(_pipe$1, to_float);
                        }
                      );
                    })();
                    if (!$1.isOk()) {
                    ...
                    let value$1 = $1[0];
                    return new Ok(new UserChangedSpaceBase(value$1));
                  }
                )
              ])
            ),
            span(toList([]), toList([text2("Space base: ")]))
          ),
          aside2(
            toList([content_last()]),
            input(
              toList([
                ...
                attribute("max", "3.0"),
                attribute("step", "0.01"),
                attribute("value", to_string4(theme.space.ratio)),
                on2(
                  "input",
                  (event2) => {
                    let $ = value(event2);
                    ...
                    let value$1 = $1[0];
                    return new Ok(new UserChangedSpaceRatio(value$1));
                  }
                )
              ])
            ),
            span(toList([]), toList([text2("Space ratio: ")]))
          ),
          aside2(
            toList([content_last()]),
            input(
              toList([
                ...
                on2(
                  ...
                  (event2) => {
                    ...
                    let value$1 = $1[0];
                    return new Ok(new UserChangedTextBase(value$1));
                  }
                )
              ])
            ),
            span(toList([]), toList([text2("Text base: ")]))
          ),
          aside2(
            toList([content_last()]),
            input(
              toList([
                ...
                attribute("max", "3.0"),
                attribute("step", "0.01"),
                attribute("value", to_string4(theme.text.ratio)),
                on2(
                  "input",
                  (event2) => {
                    let $ = value(event2);
                    ...
                    let value$1 = $1[0];
                    return new Ok(new UserChangedTextRatio(value$1));
                  }
                )
              ])
            ),
            span(toList([]), toList([text2("Text ratio: ")]))
          ),
          aside2(
            toList([content_last()]),
            input(
              toList([
                ...
                on2(
                  ...
                  (event2) => {
                    ...
                    let value$1 = $1[0];
                    return new Ok(new UserChangedRadius(value$1));
                  }
                )
              ])
            ),
            span(toList([]), toList([text2("Radius: ")]))
          ),
          p(toList([]), toList([text("Buttons:")])),
          cluster2(
            toList([stretch()]),
            toList([
              button3(toList([clear2()]), toList([text("these")])),
              button3(toList([soft()]), toList([text("are")])),
              button3(toList([solid()]), toList([text("some")])),
              button3(toList([error2()]), toList([text("buttons")])),
              button3(
                toList([success()]),
                toList([centre2(toList([]), share(toList([])))])
              ),
              button3(
                toList([greyscale()]),
                toList([centre2(toList([]), heart(toList([])))])
              )
            ])
          ),
          p(toList([]), toList([text("Button groups:")])),
          group2(
            toList([]),
            toList([
              button3(toList([]), toList([text("these")])),
              button3(toList([]), toList([text("are")])),
              button3(toList([]), toList([text("some")])),
              button3(toList([]), toList([text("buttons")]))
            ])
          ),
          p(toList([]), toList([text("Tags:")])),
          cluster2(
            toList([]),
            toList([
              tag2(toList([]), toList([text("erlang")])),
              tag2(toList([]), toList([text("elixir")])),
              tag2(
                toList([attribute("tabindex", "0")]),
                toList([text("gleam"), cross(toList([]))])
              ),
              tag2(toList([]), toList([text("javascript")]))
            ])
          ),
          p(toList([]), toList([text("Breadcrumbs:")])),
          breadcrumbs2(
            toList([]),
            caret_right(toList([])),
            toList([
              span(toList([]), toList([text("Implicit active")])),
              a(toList([]), toList([text("Page A")])),
              a(toList([]), toList([text("Page B")])),
              a(toList([]), toList([text("Page C")]))
            ])
          ),
          breadcrumbs2(
            toList([]),
            caret_right(toList([])),
            toList([
              span(toList([]), toList([text("Explicit .active class")])),
              a(toList([]), toList([text("Page A")])),
              a(toList([active()]), toList([text("Page B")])),
              a(toList([]), toList([text("Page C")]))
            ])
          ),
          p(toList([]), toList([text("Inputs:")])),
          input3(toList([])),
          p(toList([]), toList([text("...with placeholder")])),
          input3(toList([error4(), placeholder("Hi")])),
          p(toList([]), toList([text("...disabled")])),
          input3(
            toList([
              error4(),
              placeholder("Hi"),
          ...
          p(toList([]), toList([text("...clear variant")])),
          input3(toList([clear3(), placeholder("Hi")])),
          p(toList([]), toList([text("Fields:")])),
          field3(
            toList([]),
            toList([text("Name")]),
            input3(toList([])),
          ...
          field3(
            ...
            toList([text("Name")]),
            input3(toList([disabled(true)])),
            toList([text("0/200")])
          ),
          p(toList([]), toList([text("Icons:")])),
          cluster2(
            toList([]),
            toList([
              font_style(toList([])),
              ...
              font_roman(toList([])),
              font_bold(toList([])),
              font_letter_uppercase(toList([])),
              font_letter_capitalcase(toList([])),
              font_letter_lowercase(toList([])),
              font_letter_case_toggle(toList([])),
              letter_spacing(toList([])),
              align_baseline(toList([])),
              font_size(toList([])),
              font_family(toList([])),
              heading(toList([])),
              text3(toList([])),
              ...
              line_height(toList([])),
              underline(toList([])),
              strikethrough(toList([])),
              overline(toList([])),
              pilcrow(toList([])),
              text_align_left(toList([])),
              text_align_centre(toList([])),
              text_align_right(toList([])),
              text_align_justify(toList([])),
              text_align_top(toList([])),
              text_align_middle(toList([])),
              text_align_bottom(toList([])),
              dash(toList([])),
              arrow_left(toList([])),
              arrow_right(toList([])),
              arrow_up(toList([])),
              arrow_down(toList([])),
              arrow_top_left(toList([])),
              arrow_top_right(toList([])),
              arrow_bottom_left(toList([])),
              arrow_bottom_right(toList([])),
              chevron_down(toList([])),
              chevron_left(toList([])),
              chevron_right(toList([])),
              chevron_up(toList([])),
              double_arrow_down(toList([])),
              double_arrow_left(toList([])),
              double_arrow_right(toList([])),
              double_arrow_up(toList([])),
              thick_arrow_left(toList([])),
              thick_arrow_right(toList([])),
              thick_arrow_up(toList([])),
              thick_arrow_down(toList([])),
              triangle_left(toList([])),
              triangle_right(toList([])),
              triangle_up(toList([])),
              triangle_down(toList([])),
              caret_left(toList([])),
              caret_right(toList([])),
              caret_up(toList([])),
              caret_down(toList([])),
              ...
              tokens(toList([])),
              component(toList([])),
              component_alt(toList([])),
              symbol(toList([])),
              component_instance(toList([])),
              component_none(toList([])),
              component_boolean(toList([])),
              component_placeholder(toList([])),
              opacity(toList([])),
              blending_mode(toList([])),
              mask_on(toList([])),
              mask_off(toList([])),
              colour_wheel(toList([])),
              shadow(toList([])),
              ...
              zoom_in(toList([])),
              zoom_out(toList([])),
              transparency_grid(toList([])),
              group3(toList([])),
              dimensions(toList([])),
              rotate_counter_clockwise(toList([])),
              columns(toList([])),
              rows(toList([])),
              transform(toList([])),
              box_model(toList([])),
              ...
              cursor_arrow(toList([])),
              cursor_text(toList([])),
              column_spacing(toList([])),
              row_spacing(toList([])),
              play(toList([])),
              resume(toList([])),
              pause(toList([])),
              stop(toList([])),
              track_previous(toList([])),
              track_next(toList([])),
              loop(toList([])),
              shuffle(toList([])),
              speaker_loud(toList([])),
              speaker_moderate(toList([])),
              speaker_quiet(toList([])),
              speaker_off(toList([])),
              hamburger_menu(toList([])),
              close(toList([])),
              dots_horizontal(toList([])),
              dots_vertical(toList([])),
              plus(toList([])),
              minus(toList([])),
              check(toList([])),
              cross(toList([])),
              check_circled(toList([])),
              cross_circled(toList([])),
              plus_circled(toList([])),
              minus_circled(toList([])),
              question_mark(toList([])),
              question_mark_circled(toList([])),
              info_circled(toList([])),
              accessibility(toList([])),
              exclamation_triangle(toList([])),
              share(toList([])),
              share_alt(toList([])),
              external_link(toList([])),
              open_in_new_window(toList([])),
              enter(toList([])),
              exit(toList([])),
              download(toList([])),
              upload(toList([])),
              ...
              reload(toList([])),
              update2(toList([])),
              enter_full_screen(toList([])),
              exit_full_screen(toList([])),
              drag_handle_vertical(toList([])),
              drag_handle_horizontal(toList([])),
              drag_handle_dots_alt(toList([])),
              dot(toList([])),
              dot_filled(toList([])),
              commit(toList([])),
              slash(toList([])),
              circle(toList([])),
              circle_backslash(toList([])),
              half(toList([])),
              half_alt(toList([])),
              view_vertical(toList([])),
              view_horizontal(toList([])),
              view_grid(toList([])),
              view_none(toList([])),
              square(toList([])),
              copy(toList([])),
              ...
              lock_closed(toList([])),
              lock_open(toList([])),
              lock_open_alt(toList([])),
              backpack(toList([])),
              camera(toList([])),
              paper_plane(toList([])),
              rocket(toList([])),
              envelope_closed(toList([])),
              envelope_open(toList([])),
              chat_bubble(toList([])),
              link(toList([])),
              link_none(toList([])),
              link_break(toList([])),
              link_alt(toList([])),
              link_none_alt(toList([])),
              link_break_alt(toList([])),
              trash(toList([])),
              pencil(toList([])),
              pencil_alt(toList([])),
              bookmark(toList([])),
              bookmark_filled(toList([])),
              drawing_pin(toList([])),
              drawing_pin_filled(toList([])),
              sewing_pin(toList([])),
              sewing_pin_filled(toList([])),
              cube(toList([])),
              archive(toList([])),
              crumpled_paper(toList([])),
              mix(toList([])),
              mixer_horizontal(toList([])),
              mixer_vertical(toList([])),
              file(toList([])),
              file_text(toList([])),
              file_plus(toList([])),
              file_minus(toList([])),
              reader(toList([])),
              card_stack(toList([])),
              card_stack_plus(toList([])),
              card_stack_minus(toList([])),
              id_card(toList([])),
              crosshair(toList([])),
              crosshair_alt(toList([])),
              target(toList([])),
              disc(toList([])),
              globe(toList([])),
              sun(toList([])),
              ...
              clock(toList([])),
              timer(toList([])),
              counter_clockwise_clock(toList([])),
              countdown_timer(toList([])),
              stopwatch(toList([])),
              lap_timer(toList([])),
              lightning_bolt(toList([])),
              magic_wand(toList([])),
              face(toList([])),
              person(toList([])),
              eye_open(toList([])),
              ...
              eye_closed(toList([])),
              hand(toList([])),
              ruler_horizontal(toList([])),
              ruler_square(toList([])),
              clipboard(toList([])),
              clipboard_copy(toList([])),
              desktop(toList([])),
              laptop(toList([])),
              mobile(toList([])),
              keyboard(toList([])),
              ...
              activity_log(toList([])),
              bar_chart(toList([])),
              divider_vertical(toList([])),
              divider_horizontal(toList([])),
              modulz_logo(toList([])),
              switches_logo(toList([])),
              figma_logo(toList([])),
              framer_logo(toList([])),
              sketch_logo(toList([])),
              twitter_logo(toList([])),
              ...
              github_logo(toList([])),
              vercel_logo(toList([])),
              codesandbox_logo(toList([])),
              notion_logo(toList([])),
              discord_logo(toList([])),
              instagram_logo(toList([])),
              linkedin_logo(toList([])),
              border_all(toList([])),
              border_split(toList([])),
              border_left(toList([])),
              border_right(toList([])),
              border_top(toList([])),
              border_bottom(toList([])),
              corners(toList([])),
              ...
              border_style(toList([])),
              border_solid(toList([])),
              border_dashed(toList([])),
              border_dotted(toList([])),
              alignment_top(toList([])),
              alignment_centre_vertically(toList([])),
              alignment_bottom(toList([])),
              stretch_vertically(toList([])),
              alignment_left(toList([])),
              alignment_centre_horizontally(toList([])),
              alignment_right(toList([])),
              stretch_horizontally(toList([])),
              space_between_horizontally(toList([])),
              space_even_horizontally(toList([])),
              space_between_vertically(toList([])),
              space_even_vertically(toList([])),
              pin_left(toList([])),
              pin_right(toList([])),
              pin_top(toList([])),
              pin_bottom(toList([]))
          ...
          alert2(
            ...
            toList([
              aside2(
                toList([content_last(), align_centre()]),
                p(toList([]), toList([text("This is an alert!")])),
                info_circled(toList([]))
              )
            ])
          ),
          alert2(
            ...
            toList([
              aside2(
                toList([content_last(), align_centre()]),
                p(toList([]), toList([text("Ooo this one is scary.")])),
                exclamation_triangle(toList([]))
              )
            ])
          ),
          prose2(
            toList([full()]),
            toList([
              h1(toList([]), toList([text("Prose:")])),
              h2(toList([]), toList([text("A demo")])),
              p(
                toList([]),
                toList([
                  text(
              ...
              h3(toList([]), toList([text("It even supports lists!")])),
              ul(
                toList([]),
                toList([
                  li(toList([]), toList([text("Unordered list item 1")])),
                  li(toList([]), toList([text("Unordered list item 2")])),
                  li(toList([]), toList([text("Unordered list item 3")]))
                ])
              ),
              p(
                toList([]),
              ...
              ol(
                toList([]),
                toList([
                  li(toList([]), toList([text("Gleam")])),
                  li(toList([]), toList([text("Elm")])),
                  li(toList([]), toList([text("JavaScript")]))
                ])
              ),
              h3(toList([]), toList([text("Inline elements")])),
              p(
                toList([]),
                toList([
                  text("Inline elements like "),
                  a(
                    toList([href("https://www.google.com")]),
                    toList([text("links")])
                  ),
                  text(" and "),
                  em(toList([]), toList([text("emphasis")])),
                  text(" are also styled nicely. "),
                  strong(toList([]), toList([text("Strong")])),
                  text(" text is also supported, as is "),
                  code(toList([]), toList([text("code")])),
                  text(" and "),
                  kbd(toList([]), toList([text("keyboard")])),
                  text(" text.")
                ])
              ),
              h3(toList([]), toList([text("Blockquotes")])),
              p(
                toList([]),
                toList([
                  text(
              ...
              h3(toList([]), toList([text("Code blocks")])),
              p(
                toList([]),
                toList([
                  text(
              ...
              h3(toList([]), toList([text("Images")])),
              img(
                toList([src("https://source.unsplash.com/random")])
              )
            ])
          )
        ])
...
function main() {
  let app = simple(init2, update3, view);
